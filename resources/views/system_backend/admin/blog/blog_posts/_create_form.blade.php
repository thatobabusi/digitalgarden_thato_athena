    ...
    <div class="row">
            ...
            <label class="required" for="user_id">{{ trans('cruds.blogPost.fields.author') }}</label>
                ...
                <select class="form-control select2 {{ $errors->has('user_id') ? 'is-invalid' : '' }}"
                        ...
                        @foreach($users as $id => $author)
                            <option value="{{ $id }}"
                                {{ isset($blogPost->id) && $blogPost->user_id == $id ? 'selected' : '' }}
                            >
                                {{ $author }}
                            </option>
                        @endforeach
        ...
        <div class="form-group col-md-6">
            ...
            <select class="form-control select2 {{ $errors->has('blog_post_category_id') ? 'is-invalid' : '' }}"
                    name="blog_post_category_id" id="blog_post_category_id" required>

                @foreach($categories as $id => $category)
                    <option value="{{ $id }}"
                        {{ isset($blogPost->id) && $blogPost->blogPostCategory->id == $id ? 'selected' : '' }}>
                        {{ $category }}
                    </option>
                @endforeach
            </select>
        ...
        <div class="form-group col-md-12">
            ...
            <select class="form-control select2 {{ $errors->has('tags') ? 'is-invalid' : '' }}" name="tags[]" id="tags" multiple required>
                @foreach($tags as $id => $tag)
                    <option value="{{ $id }}" {{ (in_array($id, old('tags', [])) || (isset($blogPost->id) && $blogPost->blogPostTags->contains($id)) )? 'selected' : '' }}>
                        {{ $tag }}
                    </option>
                @endforeach
            </select>
