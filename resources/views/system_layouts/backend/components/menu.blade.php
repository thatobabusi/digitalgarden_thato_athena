        ...
        <ul class="nav">
            ...
            @if(plugin_is_enabled('Access Control Management'))
                @can('access_control_management_access')
                    @include('system_layouts.backend.partials.menu_partials.access_control')
                @endcan
            @endif

            @if(plugin_is_enabled('Activity Log'))
                @can('activity_log_access')
                    @include('system_layouts.backend.partials.menu_partials.activity_log')
                @endcan
            @endif

            @if(plugin_is_enabled('Blog'))
                @can('blog_management_access')
                    @include('system_layouts.backend.partials.menu_partials.blog')
                @endcan
            @endif
                    ...
                    @if(plugin_is_enabled('Pages Manager'))
                        <li class="nav-item">
                            <a href="{{route('admin.system-pages.index')}}" class="nav-link {{ request()->is('admin/crud-generator') || request()->is('admin/crud-generator/*') ? 'active' : '' }}">
                                <i class="fa-fw fas fa-ban nav-icon"></i>
                                Pages Manager
                            </a>
                        </li>
                    @endif
            ...
            @if(plugin_is_enabled('Developer Tools'))
                @can('developer_tools_access')
                    @include('system_layouts.backend.partials.menu_partials.developertools')
                @endcan
            @endif

            @if(plugin_is_enabled('User Management'))
                @can('user_management_access')
                    @include('system_layouts.backend.partials.menu_partials.uploads')
                @endcan
            @endif

            @if(plugin_is_enabled('User Management'))
                @can('user_management_access')
                    @include('system_layouts.backend.partials.menu_partials.users')
                @endcan
            @endif
